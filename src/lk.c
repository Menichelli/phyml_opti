      ...
      for(int site=0;site<(int)n_patterns;++site)
        {
          if(p_lk_loc[site] != site)
            {
              PhyML_Copy_Subpatt_Partials_Range(tree,p_lk,sum_scale,p_lk_loc,
                                                ncatg,ns,
                                                (unsigned int)site,
                                                (unsigned int)site + 1U);
            }
        }
    }
}
  ...
  for(i=0;i<2*tree->n_otu-1;++i)
    {
      ...
      tree->a_edges[i]->n_subpatt_left = (unsigned int)tree->n_pattern;
      tree->a_edges[i]->n_subpatt_rght = (unsigned int)tree->n_pattern;
    }
